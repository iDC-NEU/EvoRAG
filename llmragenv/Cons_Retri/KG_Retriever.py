...
class RetrieverGraph(object):
    ...
    def retrieve_path_with_keywords_v2(self, question, triplets_score, keywords = [], path_depth = 2, pruning = None, pruning2 = 30, threshold=0.65, score_threshold = 95, build_node = False):
        self.pruning = pruning
    ...
    def process_redundant_relationship_v2(self, parsed_response_for_relationship, redundant_relationship_3d, TF):
        # print(f"KG_Retriever: 111")
        if not parsed_response_for_relationship:
            return "", []
        # print(f"KG_Retriever: 222")
        keep_str, delete_list, *extra_values = self.graph_database.process_redundant_relationship_v2(parsed_response_for_relationship, redundant_relationship_3d, TF)
        # print(f"KG_Retriever: 333")
        # print("Keep List Log:", log)
        # print("Delete List:", delete_list)
        # print("Extra Values:", extra_values)
        return keep_str, delete_list
    ...
    def extract_keywords_with_embedding_find_entity(self, question, max_keywords=5 ):        
        ...
        sorted_all_entity_scores = self.semantic_pruning_entities_batch(question, entities_list, rel_embeddings=entity_embeddings, topk=max_keywords, batch_size=8)
    ...
    def semantic_pruning_sentences_batch(self, question, sentences, rel_embeddings=None, topk=30, batch_size=8):
        # get_text_embeddings 已经批处理 这里只是余弦相似度计算批处理，作用不大
        question_embed = np.array(get_text_embedding(question)).reshape(1, -1)
